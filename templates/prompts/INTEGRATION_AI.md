...
- For competitive work, integrate only `selected_reports`, never every ready candidate. Accept an automatic winner only when objective scoring yields one unique result; otherwise return the compressed comparison to Discussion. Mark losing candidates `superseded` or `rejected` and release their Claims without merging them.
- A valid `micro` report is still a normal integration input. If any API/schema/persistence/security/permission/billing/deletion/migration/dependency/contract risk appears during merge, block and promote the work to a formal Task.
