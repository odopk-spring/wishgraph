...
Then classify the first Task and name its exact file. Recommend a Worker profile from the user's stated quality, speed, cost, quota, and model availability plus this Task's complexity and risk. Never use one universal model or effort for every user. Store only grounded host-specific recommendations in the Task state's `worker_execution_profiles`; omit a host when the current default is the only honest choice. Move Flow Phase to `awaiting_worker_authorization` and set the unique `expected_transition` to `approve_worker_launch(<task-id>)`. Ask in one plain sentence, for example: `Will execute 012b with the recommended terra / xhigh profile. Reply Approve to use it, or Execute 012b sol high to override.` When that transition is unique, `批准`, `可以`, `开始吧`, `执行吧`, `继续`, `按这个做`, or `创建吧` authorizes only that Worker launch. A plain approval uses this Task's recommendation, or the actual host default when none was recorded; an exact execution command may override model and reasoning effort. Multiple waiting Tasks require an exact ID.

After authorization, move to `routing_worker`. Ask the active Codex host to create the project `wishgraph-worker` in a user-visible and inspectable Agent thread named `<task-id> · <short title> · WG Worker`; Claude Code prefers its managed background Worker when capability checks pass. Persist `waiting_for_worker` only after a real thread/session ID is saved. An unknown host or failed creation moves to `waiting_for_user_launch` and prints the Host Adapter's compact handoff: the exact project directory, copy-ready Codex and Claude Code startup commands with each configured profile, and the one-line Task command. Stop execution actions in Discussion. Hooks prepare and persist routes but never create Agents. Never invent launch flags or implement the Task here.
