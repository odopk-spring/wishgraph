...
讨论 AI 先判断工作类型；人类明确命令后，宿主支持时由讨论 Agent 创建并配置用户可见 Worker，手动复制仅作降级。安全串行和机械检查证明独立的 `parallel_independent` 结果静默集成；高风险、冲突、阻塞、竞争或不明确结果返回 Discussion。集成 Agent 是临时角色，并按宿主能力如实降级。
