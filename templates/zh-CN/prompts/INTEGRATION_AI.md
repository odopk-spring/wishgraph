...
- 对每个已吸收 Task，只有绑定的规范 Run、已释放 Claim、准确 Worker commit 和不可变报告共同证明完整执行过程时，才把持久 task-state 从 `draft` 或 `approved` 直接改为 `integrated`。Revision 集成后改为 `integrated`。只有后续 Discussion review 才能改为 `reviewed`。
- 只有集成后的项目事实变化时才更新 PRD、架构、CODEMAP 和 conventions。
- 角色 prompt 保持稳定；当前用户可读状态只写入 `reports/PROJECT_STATUS.md`。
