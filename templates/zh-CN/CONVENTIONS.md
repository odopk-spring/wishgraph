...
- 持久 task-state 通常使用 `draft -> approved -> integrated -> reviewed`。Git common dir 中的规范 Run 记录派发、运行、终态证据与集成；旧 Task 执行状态继续兼容。
...
- 持久 Task Lifecycle：`draft`、`approved`、`integrated`、`reviewed`，旧的临时标签继续兼容；规范 Run Phase：`dispatching`、`running`、`succeeded`、`failed`、`decision_required`、`integrating`、`integrated`。
