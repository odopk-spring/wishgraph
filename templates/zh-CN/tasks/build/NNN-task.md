...
下面的 JSON 块是机器可读任务生命周期真相源。用户明确授权创建这个用户可见且可检查的 Worker thread 或窗口前，保持 `worker_creation_authorized` 为 false。安全 `parallel_independent` 可沿用 Worker 授权；`high_risk` 的 `integration_policy` 使用 `requires_explicit_user_confirmation`。
