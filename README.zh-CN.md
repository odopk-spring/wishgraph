...
Worker 通过准确 Task 检查后才会修改代码；`enforce` 还要求先取得 Claim，`warn` 不因 Claim 自动化缺失而阻塞。它不会读取无关 Task、全部历史报告或完整源码树。

这里的“3 秒派发”指的是：精确命令被解析、授权写入规范 Run，并产生可执行的宿主路由。原生 thread/session 创建和模型启动可能更慢；严格模式在取得真实 ID 和 Claim 前只显示 `starting` / `awaiting_claim`，建议模式可在自动化缺失时按准确批准的 Task 继续。
...
以上命令固定安装不可变的 `v0.1.2` 正式标签。只有测试尚未发布的开发版本时，才从
`main` 下载安装器并显式设置 `WISHGRAPH_REF=main`。
...
默认 `warn` 是完全非阻断的建议模式：Hook、回执、Claim 和闭环问题都保持安静，不影响任务分发和普通工具。只有明确使用 `--strict`（PowerShell 为 `-Strict`）时才开启机械门禁。
...
宿主没有加载 Adapter 时，`warn` 仍然可以把已批准任务交给可见 Worker；`enforce` 才要求 Adapter 和近期 Hook 回执。两种模式都不是操作系统沙箱。
...
- **严格模式 Claim 绑定：**`enforce` 要求业务写入和构建前把 Claim 绑定到 Task、session、branch、绝对 worktree、scope 和 validation plan；`warn` 保留准确 Task 边界，但不因 Claim 自动化缺失而阻塞。
- **可检查 Worker：**只有拥有稳定 thread/session ID、独立上下文、可查看和可控制过程的 Agent 才能成为 Formal Worker；隐藏子代理只能做只读辅助。
- **单写者状态：**Worker 在 Run Report 中提出共享状态更新，只有 Discussion-local Integration 能更新共享项目事实；`enforce` 还要求绑定 Integration lease。
- **真实终态：**自然语言“完成了”不算证据；Task 状态、Run Report、验证结果和 result commit 必须一致，已经取得的 Claim 也必须正常关闭。
...
- 兼容性与 Beta 限制：[COMPATIBILITY.md](COMPATIBILITY.md)
- 安全问题报告：[SECURITY.md](SECURITY.md)
- 贡献指南：[CONTRIBUTING.md](CONTRIBUTING.md)
- 本地数据与隐私：[PRIVACY.md](PRIVACY.md)
