...
- 安装一个用户级 Adapter；未明确启用 WishGraph 的项目中它保持静默。团队也可明确选择项目级 Hooks。
...
| `manual_command_only` | 输出项目目录、Codex/Claude 可复制启动命令和最后的 `执行 <task-id> 任务`；Discussion 随即停止执行动作。 |
...
`claude --bg` 返回不代表 Task 已经 `running`。`enforce` 要求保存真实 session ID，并取得绑定 Task、session、branch、绝对 worktree、scope 和 validation plan 的 Claim；`warn` 缺少这类自动化时可按准确批准的 Task 继续。

任何检测或启动失败都严格降级为可复制的跨宿主交接，末行保持：
...
WishGraph 只有在 Task 终态、不可变 Run Report、验证结果和结果 commit 相互一致时，才进入 Integration；`enforce` 还要求已释放 Claim 的证据。
...
- 一个 Worker 同一时间只能绑定一个 Task 或 Revision；复用前释放仍有效且已经取得的 Claim，stale 记录不阻止替代 Worker。
