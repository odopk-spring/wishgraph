...
- Worker sessions use separate branches or worktrees, create one immutable `reports/runs/<work-unit-id>-attempt-N.md`, and record Integrate or N/A proposals without editing shared memory.
- Discussion-local Integration uses a bound lease in `enforce`; in `warn`, it may integrate a verified visible Worker result directly. It rewrites `reports/PROJECT_STATUS.md` in the integration commit.
- Worker creation requires an explicit human command. In `warn`, missing Hook or Claim automation does not block a visible Worker from receiving the exact approved Task. In `enforce`, keep the native background Worker, real identity, Worktree, and Claim contract. Discussion never implements as fallback.
- Route a lightweight Revision to an eligible bound Worker when the host can inspect and steer it; otherwise create an eligible Worker route or print the exact Revision handoff. Release any non-stale acquired Claim before reuse. `enforce` acquires the Revision's new scope/validation Claim; `warn` may continue from the exact Revision without it.
...
- Hooks are advisory in `warn` and enforce boundaries only in `enforce`; they do not choose parallelism, launch agents, merge code, write semantic memory, or replace review.
...
- Prefer one commit, but allow any bounded linear commit range per completed Task-backed execution unit.
