...
新任务规格包含 `wishgraph:task-state`，执行报告包含 `wishgraph:run-state`，项目状态快照包含 `wishgraph:integration-state`。持久 Task 通常只走 `draft -> approved -> integrated -> reviewed`；Git common dir 中的规范 Run 负责派发、运行、终态证据与集成进度。旧 Task 执行状态继续兼容。Hooks 只有在准确 Run、Claim、Worker commit 和报告证据齐全时才允许直接集成，因此 main 不再需要人为补写中间生命周期提交。
