...
默认长度限制用于保持快照可用：项目状态概览最多 160 行、12000 字符，讨论动态区最多 30 行，SessionStart 注入最多 2000 字符。项目状态概览任一限制超出时，`warn` 会提醒压缩但不阻止，`enforce` 会阻止集成结束和提交。历史细节应移动到单次执行报告和 Git 历史；不能为了满足限制而删除仍未解决的风险、冲突或待决定事项。
