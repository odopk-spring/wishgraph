...
4. Use the Claude session ID supplied by the WishGraph route context. Run the exact execution preflight and acquire a Worker Claim bound to that Task, session, current branch, and absolute worktree before any business write, dependency install, build, or implementation test.
...
- Modify only the assigned Task scope. Do not redesign, expand scope, start another Worker, or perform Discussion-local Integration.
